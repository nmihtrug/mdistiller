...
def wsld_weight(logits_student, logits_teacher, target, num_classes):
...
class DKD_WSLD(Distiller):
    """Decoupled Knowledge Distillation(CVPR 2022)"""

    def __init__(self, student, teacher, cfg):
        super(DKD_WSLD, self).__init__(student, teacher)
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        wsld = wsld_weight(
            logits_student, logits_teacher, target, kwargs["num_classes"]
        )
        
        # losses
        loss_ce = self.ce_loss_weight * F.cross_entropy(logits_student, target)
        loss_tckd, loss_nckd, loss_dkd = dkd_loss(
            logits_student,
            logits_teacher,
            target,
            self.alpha,
